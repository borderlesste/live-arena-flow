...
import { competitions, getTeam, highlights } from "@/data/mocks";
...
const MatchDetailsPage = () => {
  ...
  const comp = match ? competitions.find((c) => c.id === match.competitionId) : null;
  ...
  const relatedHighlights = highlights.filter((h) => h.matchId === match.id);
  const canShow = match.status === "live" || match.status === "scheduled";
    ...
    <section className="container mx-auto space-y-6 px-4 py-6 md:px-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_400px]">
        <div className="space-y-4">
          {canShow ? (
            <LivePlayer match={match} homeTeam={home} awayTeam={away} competitionName={comp.name} />
          ) : (
          ...
          {relatedHighlights.length > 0 ? (
            <div>
              <h2 className="mb-3 font-display text-xl font-bold">Destacados</h2>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {relatedHighlights.map((h) => <VideoHighlightCard key={h.id} highlight={h} />)}
              </div>
            </div>
          ) : null}
        </div>
        {isDesktop ? (
          <ChatPanel className="h-full max-h-[680px]" matchTitle={`${home.shortName} vs ${away.shortName}`} />
        ) : (
          <Sheet>
            ...
            <SheetContent side="bottom" className="h-[85vh] bg-card p-0">
              ...
              <div className="h-[calc(85vh-57px)]"><ChatPanel className="h-full rounded-none border-0" /></div>
            </SheetContent>
          </Sheet>
        )}
      </div>
