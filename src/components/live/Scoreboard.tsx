...
export function Scoreboard({ match, homeTeam, awayTeam, competitionName, variant = "overlay" }: ScoreboardProps) {
  ...
  return (
    <div className="pointer-events-none flex w-full items-start justify-between gap-3 p-3 md:p-4">
      <div className="flex items-center gap-2">
        {isLive ? <LiveBadge /> : null}
        <span className="rounded-md bg-black/40 px-2 py-0.5 text-[11px] font-medium uppercase tracking-wider text-foreground/90 ring-1 ring-white/10">
          {competitionName}
        </span>
      </div>
      <div className="flex items-center gap-3 rounded-md bg-black/45 px-3 py-1.5 text-sm ring-1 ring-white/10" aria-live="polite">
        <span className="flex items-center gap-1.5">
          <TeamBadge team={homeTeam} size="sm" />
          <span className="hidden font-semibold sm:inline">{homeTeam.shortName}</span>
        </span>
        <span className="font-display text-base font-bold tabular-nums">{match.homeScore} : {match.awayScore}</span>
        <span className="flex items-center gap-1.5">
