...
async function main() {
  ...
  if (!roleArg) {
    console.error('Uso: node scripts/grant-role.js <role> [user-id|email]');
    process.exit(2);
  }
  ...
  console.log(`Asignando rol '${roleArg}' al usuario ${user.email} (${userId})`);

  const endpoint = `${base.replace(/\/$/, '')}/rest/v1/user_roles`;
  const payload = { user_id: userId, role: roleArg, granted_by: null };
