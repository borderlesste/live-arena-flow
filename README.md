...
Las migraciones Supabase están en `supabase/migrations`. El frontend activa Supabase Auth al configurar `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY`; en producción la API exige también `SUPABASE_URL`, `SUPABASE_ANON_KEY` y `STREAM_SECRET_KEY`.
