...
Las migraciones Supabase están en `supabase/migrations`. El frontend activa Supabase Auth al configurar `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`; en producción la API exige también `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY` y `STREAM_SECRET_KEY`.
